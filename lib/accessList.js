...
class AccessList extends MerkleTree {
    constructor({
        ...
        switch (treeType) {
            ...
            case "blocklist":
                ...
                break;
            default:
                throw new Error(`Invalid treeType: ${treeType}. Use "allowlist" or "blocklist".`);
        }

        if (!/^[01]*$/.test(subsetString)) {
            throw new Error(`Invalid subsetString: ${subsetString}. Use a binary string only.`);
        }
    ...
    resizeTreeIfNecessary(index) {
        if (typeof index !== 'number' || index < 0 || index >= this.capacity) {
            throw new Error(`Invalid index: ${index}. Use an index in the range of the tree capacity.`);
        }
        const numLeaves = this.elements.length;
        if (numLeaves < (index + 1)) {
            try {
                this.bulkInsert(zeroArray(index + 1 - numLeaves, this.zeroElement));
            } catch (err) {
                console.error(err);
