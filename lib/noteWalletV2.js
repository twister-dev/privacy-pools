...
class NoteWalletV2 {
    ...
    newInteriorKeysFromPath(index) {
        const { secret, commitment } = this.interiorKeys[index];
        if (
    ...
    newExteriorKeysFromPath(index) {
        const { secret, commitment } = this.exteriorKeys[index];
        if (
            typeof secret === "undefined" ||
            typeof commitment === "undefined"
        ) {
    ...
    interiorNullifierAt(index, leafIndex) {
        const { secret } = this.interiorKeys[index];
    ...
    exteriorNullifierAt(index, leafIndex) {
        const { secret } = this.exteriorKeys[index];
        if (typeof secret === "undefined") {
            this.newExteriorKeysFromPath(index);
        }
